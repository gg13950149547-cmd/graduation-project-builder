...
- `scripts/validate_structural_figure_geometry.py` is the canonical source-scale geometry validator for ER/dense structural figures; it writes the figure geometry report consumed by `thesis_figure_contract.py` and final acceptance evidence.
...
- use them only when the current task needs figure, formula, table, or TOC visual imitation
