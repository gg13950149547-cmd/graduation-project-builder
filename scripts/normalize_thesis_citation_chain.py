...
def bibliography_entry_numbers(paragraphs: list[ET.Element], biblio_idx: int) -> list[int]:
    numbers: list[int] = []
    for para in paragraphs[biblio_idx + 1 :]:
...
def replace_bibliography_number_preserving_runs(paragraph: ET.Element, number: int) -> None:
    text = paragraph_text(paragraph)
    match = re.match(r"^\[(\d+)\]", text)
    if not match:
        raise ValueError(f"bibliography entry does not start with a visible bracket number: {text[:80]}")
    replace_visible_prefix(paragraph, match.end(), f"[{number}]")
...
def update_bibliography_paragraph_preserving_runs(
    paragraph: ET.Element, *, number: int, bookmark_name: str, bookmark_id: int
) -> None:
    remove_existing_citation_bookmarks(paragraph)
    replace_bibliography_number_preserving_runs(paragraph, number)
...
def main() -> int:
    ...
    biblio_idx = next((idx for idx, para in enumerate(paragraphs) if is_bibliography_heading_text(paragraph_text(para))), None)
    ...
    current_paragraphs = body.findall(W + "p")
    bibliography_paragraphs = current_paragraphs[biblio_idx + 1 :]
