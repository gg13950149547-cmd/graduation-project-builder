...
6. if the rendered page still shows the old geometry, treat the replacement as failed and repair the replacement path before any further drawing pass
...
7. the chosen authoring path is draw.io source plus draw.io export
8. connector geometry will be boundary-based rather than center-to-center
9. a post-drawing collision check will be run
...
23. for ER diagrams, `scripts/validate_structural_figure_geometry.py` or the equivalent canonical figure-contract path will write a source-scale geometry report before Word insertion
...
15. every thesis structural figure is backed by a draw.io source file and a draw.io export file
